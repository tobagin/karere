# cef-clean-shutdown Specification

## Purpose

Defines the two-phase, gated window-close handshake that coordinates GTK
window dismissal with the CEF browser lifecycle so the renderer subprocess
exits cleanly before GTK tears down. Ensures the process exits with code 0
on manual close and `app.quit`, without `Couldn't release X11 display`
warnings or orphan renderer subprocesses.
