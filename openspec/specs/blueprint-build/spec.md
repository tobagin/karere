# blueprint-build Specification

## Purpose

Defines how `.blp` blueprint sources are compiled to `.ui` XML at Cargo
build time, how the resulting bundle is sourced (compiled output vs
checked-in fallback), how the host `blueprint-compiler` toolchain is
detected, and how compiler failures are surfaced to cargo.
