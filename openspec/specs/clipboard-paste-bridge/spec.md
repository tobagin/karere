# clipboard-paste-bridge Specification

## Purpose

Bridges GDK clipboard content into the embedded CEF page so that Ctrl+V pastes
images, files, and text into WhatsApp Web even though CEF's offscreen
(windowless) clipboard never consults GDK. The host intercepts Ctrl+V at the GTK
key controller, translates non-text and text GDK clipboard content into synthetic
page paste events over the M13 IPC channel, round-trips large payloads via
tempfiles under `XDG_RUNTIME_DIR`, and scopes `file://` access to those
tempfiles.
