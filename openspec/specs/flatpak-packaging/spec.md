# flatpak-packaging Specification

## Purpose

Define the requirements for packaging `gtk-cef-shell` as a Flatpak so that contributors and end users can build, install, and run the GTK + CEF shell from a single manifest with reproducible, offline, sandboxed builds.
