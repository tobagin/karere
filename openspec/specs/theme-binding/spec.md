# theme-binding Specification

## Purpose

Defines how the `theme` GSetting drives libadwaita's color scheme: the
value-to-`adw::ColorScheme` mapping applied at startup and the live
`connect_changed` handler that re-applies it without restarting the
application.
