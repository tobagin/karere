# keyboard-shortcuts-dialog Specification

## Purpose

Defines the application's keyboard shortcuts help dialog: an `AdwShortcutsDialog`
built from a blueprint composite template that enumerates every accelerator the
shell binds, grouped by category, and the `app.show-help-overlay` GAction (plus
the Ctrl+? accelerator) that presents it over the active window.
