# flatpak-finalize Specification

## Purpose

Define the requirements for finalizing the Karere v4 Flatpak so that it ships a complete, lint-clean, offline-building bundle: populated AppStream metainfo with appstream-compose enabled, a desktop entry that registers the WhatsApp scheme handler, minimal-yet-sufficient sandbox permissions (StatusNotifierWatcher for the tray, Background-portal autostart), a regenerated icon cache, an exhaustive `cargo-sources.json`, and updated README/CHANGELOG reflecting the 4.0.0 CEF/Chromium rewrite.
