# cef-input-forwarding Specification

## Purpose

Forwards GTK pointer, scroll, keyboard, and focus events from the
`CefGtkArea` widget into the embedded CEF off-screen browser host so the
rendered page responds to user input. The capability covers GTK4 event
controller wiring, GTK `ModifierType` to CEF `EVENTFLAG_*` translation,
GDK keyval to Windows virtual-key mapping, and HiDPI scale-factor
adjustment for pointer coordinates and wheel deltas.
