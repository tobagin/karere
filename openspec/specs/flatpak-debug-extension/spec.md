# flatpak-debug-extension Specification

## Purpose

Define the requirements for shipping detached debug symbols for Karere v4 as a separate, opt-in Flatpak Debug extension so that crash backtraces can be symbolicated via `coredumpctl debug karere` without bloating the base app or requiring user-side `debuginfod` configuration.
