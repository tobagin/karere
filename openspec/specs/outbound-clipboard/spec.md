# outbound-clipboard Specification

## Purpose

Publishes the embedded page's text selection to the host's GDK clipboards so
copy works outward even though CEF's offscreen (windowless) mode never owns the
system clipboard and the DOM `copy` event does not fire. The renderer mirrors
selections to the PRIMARY selection via `selectionchange`, and the GTK-level
Ctrl+C handler promotes PRIMARY to the regular CLIPBOARD.
