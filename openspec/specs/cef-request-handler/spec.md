# cef-request-handler Specification

## Purpose

Route external links to the host browser and recover gracefully from renderer crashes. Installs a CEF `RequestHandler` that intercepts top-level navigations and launches non-WhatsApp URIs through the system default handler, routes new-window/popup navigations the same way, and reacts to renderer-process termination with backoff-based auto-reload that escalates to a user-visible dialog under a crash storm.
