# tray-sni Specification

## Purpose

Defines the system-tray `StatusNotifierItem` (SNI) integration: the `ksni`-hosted tray icon, its unread-count state machine, desktop-environment auto-detect skip policy with the `KARERE_FORCE_TRAY` override, the `app.set-unread` / `app.present-window` actions that drive it, and the Flatpak permission required to register the item.
...
- **AND** the tray icon reverts to `<app-id>-tray-symbolic`
