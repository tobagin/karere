# window-state-persistence Specification

## Purpose

Defines how Karere persists window state across launches: binding window
geometry to GSettings, driving close-button behaviour from the
`close-button-action` GSetting, and gating the initial `present()` on the
`start-in-background` GSetting.
