# drag-drop-files Specification

## Purpose

Accepts files dragged from the desktop onto the embedded CEF surface and
marshals them into synthetic DOM `drop` events on the element under the cursor,
reusing the clipboard paste bridge's IPC envelope and large-payload tempfile
path so WhatsApp Web receives drag-and-dropped attachments.
