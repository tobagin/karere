...
- Introducing additional finish-args beyond StatusNotifierWatcher.
...
- **finish-args additions**: `--talk-name=org.kde.StatusNotifierWatcher` is the documented DBus name that KDE / Plasma's tray daemons expose for SNI clients; adding it is required for the M15 tray to register without portal escalation. **Autostart is NOT granted via filesystem** (revised during implementation): the originally-planned `--filesystem=xdg-config/autostart:create` and `--talk-name=org.freedesktop.portal.Desktop` were both rejected by `flatpak-builder-lint` as unnecessary — portal busnames are auto-granted, and autostart is performed host-side by the XDG Background portal through the `ashpd` `background` feature, so the sandbox never writes the autostart `.desktop` itself. Neither arg ships.
