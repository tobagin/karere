...
### Fixed
- **Stuttery scrolling and typing lag on every GPU (#173/#179)**: a CEF paint only reached the screen when a 16 ms poll timer noticed it, so each frame waited up to a full frame extra and the timer beat against the 60 Hz paint stream, silently dropping about one frame in seven and doubling the gap before the next. Paints now queue their own redraw. Measured on a wheel-scroll load: 902 of 904 paints drawn (was 776 of 902), paint-to-draw latency 0.3 ms median (was 8.6 ms), no more 32 ms double-gaps. Applies to the software and the GPU (DMA-BUF) path alike, so NVIDIA users on the forced software path get it too.
...
- **Microphone stuck muted in calls (#182)**: CEF answers media access through its permission handler, so Chromium's content settings never learned the outcome and `navigator.permissions.query` reported "prompt" forever. WhatsApp's in-call unmute preflight re-queries that state and bailed, leaving the mic muted in calls while voice notes worked. Every mic/camera grant or denial is now mirrored into the request context's content settings.
- **Characters dropped right after sending a message (#180)**: a composer re-render emits editable-focus false then true in quick succession; applying the false immediately unfocused the input method mid-typing and the next keystroke was swallowed. Focus-out is now debounced (~64 ms) so the paired focus-in cancels it, while a genuine blur still settles. IM-committed text is also encoded as proper UTF-16, so an emoji committed by the input method no longer arrives as a private-use codepoint.
- **Duplicate paste bridge listeners (#187)**: the paste bridge could be installed more than once on the same page.
- **Non-Unicode command-line arguments** no longer panic at startup (the subprocess check used `std::env::args()`, which aborts on invalid UTF-8).

### Changed
- **Browser engine upgraded to CEF 152.0.6 / Chromium 152.0.7977.83** (from CEF 150.0.10 / Chromium 150.0.7871.101): current Chromium security and rendering fixes. Proprietary codecs (H.264/AAC) stay compiled in so video attachments keep playing in-app, and the idle-CPU message-pump fix (#151) is carried over — it still applies cleanly to the 152 tree. The Rust `cef` crate moves 150.0.0+150.0.10 → 152.3.0+152.0.6.
  Chromium 152 also started requiring a EULA acceptance on Linux for a fresh user-data directory, shown through a dialog CEF cannot display, which made engine start-up fail on every first launch (exit code 28). Karere now passes `--no-first-run`, so new installs start normally.
- **CEF sandbox enabled outside Flatpak (#184)**: native (non-Flatpak) runs now use Chromium's sandbox, which needs unprivileged user namespaces; inside Flatpak nothing changes — the Flatpak sandbox stays the boundary and the flat `no-zygote` process tree is kept.
